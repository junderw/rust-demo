...
impl IntoResponse for Error {
    fn into_response(self) -> Response {
        let (code, value) = match self {
            Self::Logout(err) => (StatusCode::BAD_REQUEST, json::Error::from(err)),
            Self::Login(err) => (StatusCode::BAD_REQUEST, json::Error::from(err)),
            Self::Credentials(err) => (StatusCode::BAD_REQUEST, json::Error::from(err)),
            Self::CreateUser(err) => (StatusCode::BAD_REQUEST, json::Error::from(err)),
            Self::Auth(err) => (StatusCode::UNAUTHORIZED, json::Error::from(err)),
        };
